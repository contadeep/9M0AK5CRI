...
local aa = {
    function()
        ...
        if isMobile then
            -- Cria o botão flutuante apenas para mobile
            local showFluentButton = p.New("TextButton", {
                Text = "Hide Menu",
                Size = UDim2.fromOffset(120, 40), -- Tamanho maior para touch
                Position = UDim2.new(0, 250, 0, 100),
                ...
                p.New("UICorner", {CornerRadius = UDim.new(0, 8)}),
                p.New("UIStroke", {Color = Color3.fromRGB(100, 100, 100), Thickness = 1}),
            })
            ...
            local function startDrag(input)
                ...
                startPos = showFluentButton.Position
                
                -- Feedback visual durante arrasto
                showFluentButton.BackgroundTransparency = 0.5
                
                -- Conexão para detectar quando o toque é liberado
                local connection
                connection = input.Changed:Connect(function()
                    if input.UserInputState == Enum.UserInputState.End then
                        dragging = false
                        showFluentButton.BackgroundTransparency = 0.3
                        connection:Disconnect()
                    end
                end)
            end

            -- Função para atualizar posição durante arrasto
            local function updateDrag(input)
                if dragging then
                    ...
                    -- Limita o botão à área da tela
                    local viewportSize = workspace.CurrentCamera.ViewportSize
                    showFluentButton.Position = UDim2.new(
                        math.clamp(newPosition.X.Scale, 0, 1),
                        math.clamp(newPosition.X.Offset, 20, viewportSize.X - showFluentButton.AbsoluteSize.X - 20),
                        math.clamp(newPosition.Y.Scale, 0, 1),
                        math.clamp(newPosition.Y.Offset, 20, viewportSize.Y - showFluentButton.AbsoluteSize.Y - 20)
                    )
                end
            end

            -- Conexões de eventos para touch
            showFluentButton.InputBegan:Connect(function(input)
            ...
            local function toggleMenu()
                ...
                showFluentButton.Text = x.Window.Root.Visible and "Hide Menu" or "Show Menu"
            end

            -- Clique para alternar o menu (só executa se não estiver arrastando)
            showFluentButton.InputEnded:Connect(function(input)
                if not dragging and input.UserInputType == Enum.UserInputType.Touch then
                    toggleMenu()
                end
            end)
        end
    ...
    [16] = function()
        ...
        return function(n)
            ...
            --     o.Frame,
            --     function()
            --         p.Window.Root.Visible = not p.Window.Root.Visible
            --     end
            -- )
            -- local showText = l(
            --     "TextLabel",
            ...
            --     }
            -- )
            -- showText.Parent = o.ShowButton.Frame
            return o
        end
    end,
    [17] = function()
    ...
    [26] = function()
        ...
        function c.New(d, e, f)
            ...
            -- Botões + e -
            -- Botões + e -
            local btnSize = 20 -- Tamanho reduzido
            local btnPadding = 3 -- Espaçamento entre botões
            local minusBtn = ai(
                "TextButton",
                {
                    Size = UDim2.fromOffset(btnSize, btnSize),
                    Position = UDim2.new(1, -8, 0.5, -(btnSize / 2 + btnPadding)), -- Lado a lado com plusBtn
            ...
            local plusBtn = ai(
                ...
                {
                    Size = UDim2.fromOffset(btnSize, btnSize),
                    Position = UDim2.new(1, -8, 0.5, btnSize / 2), -- À direita do minusBtn
                    AnchorPoint = Vector2.new(1, 0.4),
                    BackgroundTransparency = 0.8,
                    BackgroundColor3 = Color3.fromRGB(120, 120, 120),
                    Text = "+",
